...
function baseTemplate(title: string, bodyHtml: string, settings: typeof defaultSettings) {
  return `
  <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;color:#0A1220;">
    <div style="background:#0B2A4A;padding:24px;text-align:center;">
      <img src="${settings.logoSrc}" alt="${settings.brandName}" width="48" style="border-radius:50%;" />
      <h1 style="color:#fff;font-size:16px;margin:12px 0 0;">${settings.brandName}</h1>
    </div>
...
export async function sendOrderConfirmationEmail(order: Order) {
  ...
  const html = baseTemplate(
    ...
    `
    <p>Hi ${order.customer.fullName}, thanks for ordering from ${settings.brandName}.</p>
  ...
  await getTransporter().sendMail({
    from: process.env.SMTP_FROM || `"${settings.brandName}" <${settings.email}>`,
